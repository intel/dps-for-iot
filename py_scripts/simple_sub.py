...
if args.encryption == 0:
    ...
    dps.set_certificate(key_store, subscriber_cert, subscriber_private_key, subscriber_password)
    dps.set_certificate(key_store, publisher_cert)
...
def on_pub(sub, pub, payload):
    ...
    if dps.publication_is_ack_requested(pub):
        ack_msg = "This is an ACK from %d" % (dps.get_listen_address(dps.publication_get_node(pub)))

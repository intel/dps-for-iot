...
static void TestLoopbackLargeMessage(DPS_Node* node, DPS_KeyStore* keyStore)
{
    ...
    ret = DPS_Publish(pub, largeMessage, A_SIZEOF(largeMessage), 0, NULL);
    ASSERT(ret == DPS_OK);
    /*
...
static void TestLoopbackAckLargeMessage(DPS_Node* node, DPS_KeyStore* keyStore)
{
    ...
    event = DPS_CreateEvent();
    ASSERT(event);
    ret = DPS_Publish(pub, message, A_SIZEOF(message), 0, NULL);
    ASSERT(ret == DPS_OK);
...
static void BackToBackPublishComplete(DPS_Publication* pub, DPS_Status status)
{
    DPS_Event* event = (DPS_Event*)DPS_GetPublicationData(pub);
    ASSERT(status == DPS_OK);
    if (DPS_PublicationGetSequenceNum(pub) < 10000) {
        DPS_Status ret = DPS_Publish(pub, NULL, 0, 0, BackToBackPublishComplete);
        ASSERT(ret == DPS_OK);
    } else {
        DPS_SignalEvent(event, DPS_OK);
    }
...
static void TestBackToBackPublish(DPS_Node* node, DPS_KeyStore* keyStore)
{
    ...
    pub = CreatePublication(node, topics, numTopics, NULL);
    ret = DPS_SetPublicationData(pub, event);
    ASSERT(ret == DPS_OK);
    ...
    for (i = 0; i < 10000; ++i) {
        ret = DPS_Publish(pub, NULL, 0, 0, BackToBackPublishComplete);
        ASSERT(ret == DPS_OK);
        ret = DPS_WaitForEvent(event);
        ASSERT(ret == DPS_OK);
    }

...
static void RemoteCompletion(void* data)
{
    ...
    if (remote) {
        ...
        if (completion->op == LINK_OP) {
            /*
             * State should be either ACTIVE, LINKING, MUTING or MUTED
             */
            if (remote->state == REMOTE_ACTIVE) {
                ...
                assert(remote->state == REMOTE_MUTED || remote->state == REMOTE_MUTING);
            }
...
DPS_Status DPS_UpdateOutboundInterests(DPS_Node* node, RemoteNode* destNode, uint8_t* changes)
{
     ...
     */
    if (destNode->state == REMOTE_MUTED || destNode->state == REMOTE_DEAD) {
        *changes = DPS_FALSE;
        return DPS_OK;
    }
    if (destNode->state == REMOTE_ACTIVE) {
        *changes = DPS_FALSE;
    } else {
        /*
         * Transition states are always a change
         */
        ret = DPS_ClearOutboundInterests(destNode);
        if (ret != DPS_OK) {
            goto ErrExit;
        }
        *changes = DPS_TRUE;

...
static const char* topic = "a/b/c/d";
...
int main(int argc, char** argv)
{
    ...
    DPS_PRINT("Starting discoverability unit test\n");
    ...
    sub = DPS_InitSubscription(node, &topic, 1);
    CHECK(sub != NULL);

    status = DPS_Subscribe(sub, OnPub, NULL);
    CHECK(status == DPS_OK);

    status = DPS_MakeDiscoverable(node, "test");
    CHECK(status == DPS_OK);

    SLEEP(120 * 1000);

    DPS_MakeNondiscoverable(node);
